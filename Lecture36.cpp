...
public:
    int longestCommonSubsequence(string text1, string text2) {
        ...
        for(int i = 0;i<n;i++){//if text2's index becomes 0 
            dp[i][0] = 0;
        }
        for(int j = 0;j<m;j++){//if text1's index becomes 0
            dp[0][j] = 0;
        }

        for(int ind1 = 1;ind1 <= n;ind1++){
            for(int ind2 = 1;ind2 <= m;ind2++){
                if(text1[ind1-1] == text2[ind2-1]){//we cant access negative indexes so shifting of indexes to right occurs
